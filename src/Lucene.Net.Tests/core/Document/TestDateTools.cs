...
namespace Lucene.Net.Document
{
    ...
    public class TestDateTools : LuceneTestCase
    {
        ...
        public virtual void TestStringToDate()
        {
            ...
            Assert.Equal("2004-01-01 00:00:00:000", IsoFormat(d));
            d = DateTools.StringToDate("20040705");
            Assert.Equal("2004-07-05 00:00:00:000", IsoFormat(d));
            d = DateTools.StringToDate("200407050910");
            Assert.Equal("2004-07-05 09:10:00:000", IsoFormat(d));
            d = DateTools.StringToDate("20040705091055990");
            Assert.Equal("2004-07-05 09:10:55:990", IsoFormat(d));

            Assert.ThrowsAny<Exception>(() => d = DateTools.StringToDate("97"));  // no date
            Assert.ThrowsAny<Exception>(() => d = DateTools.StringToDate("200401011235009999")); // no date
            Assert.ThrowsAny<Exception>(() => d = DateTools.StringToDate("aaaa")); // no date
        }
        ...
        public virtual void TestDateAndTimetoString()
        {
            ...
            Assert.Equal("2004", dateString);
            Assert.Equal("2004-01-01 00:00:00:000", IsoFormat(DateTools.StringToDate(dateString)));

            dateString = DateTools.DateToString(cal, DateTools.Resolution.MONTH);
            Assert.Equal("200402", dateString);
            Assert.Equal("2004-02-01 00:00:00:000", IsoFormat(DateTools.StringToDate(dateString)));

            dateString = DateTools.DateToString(cal, DateTools.Resolution.DAY);
            Assert.Equal("20040203", dateString);
            Assert.Equal("2004-02-03 00:00:00:000", IsoFormat(DateTools.StringToDate(dateString)));

            dateString = DateTools.DateToString(cal, DateTools.Resolution.HOUR);
            Assert.Equal("2004020322", dateString);
            Assert.Equal("2004-02-03 22:00:00:000", IsoFormat(DateTools.StringToDate(dateString)));

            dateString = DateTools.DateToString(cal, DateTools.Resolution.MINUTE);
            Assert.Equal("200402032208", dateString);
            Assert.Equal("2004-02-03 22:08:00:000", IsoFormat(DateTools.StringToDate(dateString)));

            dateString = DateTools.DateToString(cal, DateTools.Resolution.SECOND);
            Assert.Equal("20040203220856", dateString);
            Assert.Equal("2004-02-03 22:08:56:000", IsoFormat(DateTools.StringToDate(dateString)));

            dateString = DateTools.DateToString(cal, DateTools.Resolution.MILLISECOND);
            Assert.Equal("20040203220856333", dateString);
            Assert.Equal("2004-02-03 22:08:56:333", IsoFormat(DateTools.StringToDate(dateString)));
            ...
            dateString = DateTools.DateToString(cal, DateTools.Resolution.MILLISECOND);
            Assert.Equal("19610305230951444", dateString);
            Assert.Equal("1961-03-05 23:09:51:444", IsoFormat(DateTools.StringToDate(dateString)));

            dateString = DateTools.DateToString(cal, DateTools.Resolution.HOUR);
            Assert.Equal("1961030523", dateString);
            Assert.Equal("1961-03-05 23:00:00:000", IsoFormat(DateTools.StringToDate(dateString)));
        ...
        public virtual void TestDateToolsUTC()
        {
            ...
            try
            {
                ...
                Assert.False(d1.Equals(d2), "different times");
                Assert.Equal(DateTools.StringToTime(d1), time * 1000, "midnight");
                Assert.Equal(DateTools.StringToTime(d2), (time+3600) * 1000, "later");
            }
            finally
            {
            ...
            try
            {
                ...
                Assert.False(d1.Equals(d2), "different times");
                Assert.Equal(DateTools.StringToTime(d1), time1.Ticks); //, "midnight");
                Assert.Equal(DateTools.StringToTime(d2), time2.Ticks); //, "later");
            }
            finally
            {
