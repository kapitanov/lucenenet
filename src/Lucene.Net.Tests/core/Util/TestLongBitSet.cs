        ...
        internal virtual void DoRandomSets(int maxSize, int iter, int mode)
        {
            ...
            for (int i = 0; i < iter; i++)
            {
                ...
                if (b0 != null && b0.Length() <= b.Length())
                {
                    ...
                    Assert.Equal(a0.Cardinality(), b0.Cardinality());
        ...
        */
        ...
        public virtual void TestHashCodeEquals()
        {
            ...
            for (int iter = 0; iter < 10 * RANDOM_MULTIPLIER; iter++)
            {
                ...
                if (!b1.Get(idx))
                {
                    b1.Set(idx);
                    Assert.False(b1.Equals(b2));
                    Assert.False(b1.GetHashCode() == b2.GetHashCode());
                    b2.Set(idx);
                    Assert.Equal(b1, b2);
        ...
        public virtual void TestSmallBitSets()
        {
            ...
            for (int numBits = 0; numBits < 10; numBits++)
            {
                ...
                Assert.True(b1.Equals(b2));
                Assert.Equal(b1.GetHashCode(), b2.GetHashCode());
                Assert.Equal(0, b1.Cardinality());
                if (numBits > 0)
                {
                    b1.Set(0, numBits);
                    Assert.Equal(numBits, b1.Cardinality());
                    b1.Flip(0, numBits);
                    Assert.Equal(0, b1.Cardinality());
                }
            }
